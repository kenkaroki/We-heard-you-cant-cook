<script lang="ts">
	import '../../styles/page_styles/authentications/login.css';
	import {login} from "../../scripts/api/auth.ts"
	...
	// State for handling response and UI feedback
	...
	function gotodashboard() {
		...
		if (response === "good signup") {
			...
			// Wait for cake animation to complete before navigating
			setTimeout(() => {
				gotodashboard();
			}, 5000); // 5 seconds: time for cake to drop, say message, and finish
		} else {
			// Show error to user
			isLoading = false;
...
<section class="login">
	...
	<div class="food-item carrot" bind:this={carrotbody} role="img" aria-hidden="true">
			...
			<div class="sneaky-icon telescope" style="--telescope-duration: {TELESCOPE_TIME}ms;">
				...
				>
					...
					<g>
						<path
							style="fill:#FF931E;"
							d="M476.949,369.865L476.949,369.865c13.725,0,24.851-11.126,24.851-24.851V166.988
						...
						<path
							style="fill:#FF931E;"
							d="M26.918,310.222L26.918,310.222c9.233,0,16.719-7.485,16.719-16.719v-75.005
					...
					<path
						d="M476.949,131.938c-18.479,0-33.653,14.378-34.95,32.532H292.147c-5.632,0-10.199,4.567-10.199,10.199v14.199h-119.93
					...
					/>
					<path
						d="M321.065,193.388c-5.632,0-10.199,4.567-10.199,10.199v70.487c0,5.632,4.567,10.199,10.199,10.199
                        c5.632,0,10.199-4.567,10.199-10.199v-70.487C331.264,197.955,326.697,193.388,321.065,193.388z"
					/>
					<path
						d="M321.065,291.89c-5.632,0-10.199,4.567-10.199,10.199v7.229c0,5.632,4.567,10.199,10.199,10.199
		...
		{#if showNotebook}
			<span
				class="sneaky-icon notebook"
				style="animation-duration: {NOTEBOOK_TIME}ms;"
				role="img"
				aria-label="Writing in a notebook">✍️</span
			>
		{/if}
	...
	<div class="food-item cabbage" bind:this={cabbagebody} role="img" aria-hidden="true">
		...
		<div
			...
			class:visible={(showPassword && showInitialDialogue) || carrotGotPassword}
		>
			{#if carrotGotPassword}
				Got the password! Time to login! 🤫
			{:else}
				Privacy first! 🤐
			{/if}
		</div>
	...
	<div class="login-card">
		<h1>🔐 Log In</h1>
		<form method="POST" id="login-form" on:submit={handlelogin}>
			<input
				type="text"
				name="email"
					...
					{#if showPassword}
						...
						>
							<path
								...
								stroke="#333"
								stroke-width="2"
								d="M10.585 13.414a2 2 0 002.828-2.828l-2.828 2.828zm.924.924a4 4 0 01-5.656-5.656M3 3l18 18M7.3 17.5A11 11 0 0012 19c5 0 9.27-3.11 11-7-1-2.18-2.88-4.21-5.11-5.74"
							></path>
						</svg>
					{:else}
						<svg
	...
	<a href="{base}/signup" class="signup_switch">Don't have an account? SignUp</a>
	{:else}
